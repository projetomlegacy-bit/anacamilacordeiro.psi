...
export const WhatsAppButton: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Show on mobile when scrolled past 200px
      const shouldShow = window.scrollY >= 200;
      setIsScrolledPastThreshold(shouldShow);
    };
  ...
  const handleClick = () => {
    trackWhatsAppConversion("Botão Flutuante de WhatsApp (Sticky CTA)");
  };
  ...
  return (
    <a
      ...
      onClick={handleClick}
      aria-label="Agendar consulta pelo WhatsApp com a psicóloga Ana Camila Cordeiro"
      title="Falar no WhatsApp"
      className={`fixed bottom-5 right-5 z-50 md:bottom-6 md:right-8 w-[54px] h-[54px] rounded-full bg-[#25D366] text-white shadow-lg shadow-[#25D366]/40 flex items-center justify-center transition-all duration-300 ease-out hover:scale-110 hover:shadow-xl hover:shadow-[#25D366]/60 focus:outline-none focus:ring-4 focus:ring-[#25D366]/30 active:scale-95 cursor-pointer will-change-transform ${
    ...
    >
      ...
      {/* Subtle Glowing Ring */}
      <div
        className="absolute inset-0 rounded-full pointer-events-none shadow-[0_0_12px_#25D366] opacity-50"
        aria-hidden="true"
      />
    </a>
  );
